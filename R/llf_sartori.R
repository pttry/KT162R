...
llf_sartori <- function(parameters) {
  ...
  P0 <- sum(log(pnorm(-selection_param * x))[!selected])
  P1 <- sum(log((pnorm(-outcome_param * x) - pnorm(-selection_param * x))[selected & !yo][(selection_param - outcome_param)*xs[selected & !yo] > 0]))
  P2 <- sum(log(sapply(lapply(lapply(1:sum(selected & yo), function(i) matrix(c(selection_param * x[selected & yo], outcome_param * x[selected & yo]), nrow = 2, byrow = TRUE)[,i]),
                              pnorm), min)))
  ll <- P0 + P1 + P2
  ll
}
