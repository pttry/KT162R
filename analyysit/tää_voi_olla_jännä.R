...
data <- data %>% filter(grepl( "M12", Kuukausi)) %>%
                 filter(vuosi < 2017) %>%
                        mutate(tyottomyysaste = Tyottomat / Tyovoima,
                        vakanssiaste = Avoimet_tyopaikat / (Avoimet_tyopaikat + Tyovoima - Tyottomat)) %>%
                        rename(Alue = Kunta,
                               Vuosi = vuosi)
data$dTyottomat = c(NA, diff(data$Tyottomat))
data$dAvoimet_tyopaikat = c(NA, diff(data$Avoimet_tyopaikat))
...
malli <- lm(tyottomyysaste ~ vakanssiaste*dnettopendeloivat  + vuosi + Maakunta, data = data)
stargazer(malli, type = "text", omit = "Alue")
