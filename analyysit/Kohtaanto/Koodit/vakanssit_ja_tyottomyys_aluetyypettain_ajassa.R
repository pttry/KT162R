...
p1 <- df %>% ggplot(aes(x = time, y = vakanssiaste_sa, col = aluetyyppi)) +
              geom_line() +
              labs(y = "(Kausitasoitettu) vakanssiaste",
                   x = NULL,
                   color = NULL) +
              scale_color_discrete(labels = c("Kaupunki",
...
p2<- df %>% ggplot(aes(x = time, y = tyottomyysaste_sa, col = aluetyyppi)) +
  geom_line() +
  labs(y = "(Kausitasoitettu) tyottomyysaste",
       x = NULL,
       color = NULL) +
